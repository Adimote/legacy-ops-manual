...
\begin{roledesc}{team-coord}
  ...
  \item Allocating Production Kits to Teams.
  \item Allocating Local Kits to \roletitle{local-team-coord}s.
  \end{responsibilities}
\end{roledesc}
...
\begin{roledesc}{local-team-coord}
  ...
  \item Recruiting new teams in the area they are responsible for.
  \item Reporting broken or damaged kit to the \role{kit-support-coord}.
  \end{responsibilities}
...
\begin{roledesc}{mentors}
  ...
  \begin{responsibilities}
  \item Assisting a team by consulting them about issues they are having, or may have, that relate to their involvement in the competition.
  \item Regularly meeting with their team.
  \item Providing a report to their supervisor when they have visited a team.
  \item Notifying their supervisor if they are unable to mentor a team for a significant period of time.
  \end{responsibilities}
\end{roledesc}
...
\begin{roledesc}{kit-coord}
  \begin{responsibilities}
    ...
    \item Approve the technical and safety regulations in the rules of the next competition year's Game.
    \item Allocate Development Kits and development tools
    \item Approve final hardware (electronic and mechanical) prior to manufacture and inclusion in the kit.
    \item Evaluate potential legal implications of software (local and remote) prior to release.
    \item Advise \role{local-team-coord}s of the couriering details for kits not collected at Kickstart.
  \end{responsibilities}
\end{roledesc}

\begin{roledesc}{hw-dev-coord}
  \begin{responsibilities}
    \item{Research of new hardware ideas.}
    \item{Development and testing of new hardware.}
    \item{Debugging of existing hardware.}
    \item Approval of new kit proposals to release money for development.
    \item{The creation and maintenance supporting documentation for hardware including:
        \begin{itemize}
...
\begin{roledesc}{hw-prod-coord}
  \begin{responsibilities}
    \item{Manufacture of new hardware ready for inclusion in the kit.}
    \item Manufacture of whole new kits (all classes) in preparation for the next Competition Programme Year.
    \item Ensuring that spare kit is replenished as it is used up.
...
\begin{roledesc}{kit-sw-coord}
  ...
  \begin{responsibilities}
    \item{Development of kit related software including:
      \begin{itemize}
        \item{Software that will run on the kit hardware, e.g. firmware, and python libraries. Known as Kit Local Software}
        \item{Software that aids teams with developing their robots but does not run on the kit hardware, e.g. IDEs, debuggers, and simulators. Known as Kit Remote Software}
      \end{itemize}
    \item Defining the overall architecture of SR software such that software falls into clearly defined projects.
    }
    \item Defining certain projects as critical - those which are difficult, if not impossible, to update during the Competition Programme Year.
    \item Approving changes to critical software projects.
    \item{Writing of support documentation including:
      \begin{itemize}
        \item{APIs that the kit provides to teams.}
        \item{Kit software tutorials.}
        \item{APIs of software that aids teams with developing their robots.}
      \end{itemize}
    }
  \end{responsibilities}
\end{roledesc}
...
\begin{roledesc}{kit-logistics-coord}
  \begin{responsibilities}
    \item{Managing the inventory of SR assets including:}
      \begin{itemize}
        \item{Ensuring that the inventory is representative of reality.}
        \item{Removal of assets from the inventory after disposal.}
      \end{itemize}
    \item Storage, tracking (updating physical location in the inventory) and shipping of all assets including:
      \begin{itemize}
        \item{Ensuring that all localities hosting a Kickstart event have the correct Kits shipped to them prior to Kickstart.}
        \item{Shipping replacement parts to Teams when instructed to do so by the \role{kit-support-coord}.}
        \item Shipping of non-kit assets such as competition hardware.
      \end{itemize}
    \item Ensuring that kits non collected at Kickstart are available for courier collection no later than two weeks after Kickstart.
    \item{Securing the timely return of Kits not handed back at the Competition.}
  \end{responsibilities}
\end{roledesc}

\begin{roledesc}{kit-support-coord}
  \begin{responsibilities}
    \item Provide support to users of the Kit including both teams and volunteers.
    \item Manage allocation of Support Kit.
    \item Assess claims of broken Kit and authorise shipping of spare kit where required.
  \end{responsibilities}
\end{roledesc}
...
\begin{roledesc}{ancillary-sw-coord}
  ...
  \item{Development and operation of ancillary (non-kit) team facing software including:
    \begin{itemize}
      \item{Forum.}
      \item{Team management software, which includes software for managing team registration, user management, media consent form submissions, and ticketing.}
    \end{itemize}
  }
  ...
  \item{Writing of support documentation for the systems that the \roletitle{ancillary-sw-coord} is responsible for.}
  \end{responsibilities}
\end{roledesc}
...
\begin{roledesc}{reward-manager}
  The \roletitle{reward-manager} is responsible for procuring the rewards associated with each of the competition awards.
\end{roledesc}
...
\begin{roledesc}{prod-manager}
  The \roletitle{prod-manager} is responsible for the set-up, maintenance, and get-out of the venue used for the competition, and all physical installations within it.

  \begin{responsibilities}
  \item{Finding a suitable venue for the competition, managing its booking, and communicating with venue staff.}
  \item{The logistics of getting things to the venue, as well as taking them away.}
    \item{Hiring and interacting with any contractors brought-in for the event.}
    \item{Set-up of the arenas and pits at the event, including sound systems, networking, power and lighting.}
    \item{Obtaining suitable insurance and licenses for the event.}
  \end{responsibilities}
\end{roledesc}

\begin{roledesc}{comp-team-coord}
  The \roletitle{comp-team-coord} is responsible for managing the teams at the competition.

  \begin{responsibilities}
  \item Ensuring teams are well-informed about the event whilst they are at the event, including the site rules, how they should get to a match, and where their pits are.
    \item{Kit collection from teams.  The \roletitle{comp-team-coord} should interact with the \role{kit-logistics-coord} regarding this.}
    \item Reception, which includes:
      ...
      \end{itemize}
    \item{Managing team access to the arenas during tinker time.}
  \end{responsibilities}
\end{roledesc}

\begin{roledesc}{comp-sw-coord}
  The \roletitle{comp-sw-coord} is responsible for the development, maintenance, and operation of the competition software.

  \begin{responsibilities}
  \item{Software services that present information to teams throughout the competition.}
  \item{Score entry and management systems.}
  \item{Interacting with the \role{prod-manager} regarding hardware and networking requirements.}
  \end{responsibilities}
\end{roledesc}

\begin{roledesc}{comp-team-support-coord}
  The \roletitle{comp-team-support-coord} is responsible for providing teams with technical support throughout the competition.

  \begin{responsibilities}
  \item Managing technical support services provided to teams, e.g. helpdesk, and battery charging.
  \end{responsibilities}
\end{roledesc}

\begin{roledesc}{comp-hs-coord}
  The \roletitle{comp-hs-coord} is responsible for ensuring that teams, and SR volunteers are safe throughout the competition.

  \begin{responsibilities}
  \item Safety-checking robots before they are allowed to compete within the competition.
  \item Ensuring that robots continue to be safe throughout the competition.
  \item Arranging for first-aid facilities at the competition.
  \end{responsibilities}
\end{roledesc}

\begin{roledesc}{comp-match-coord}
  The \roletitle{comp-match-coord} is responsible for running matches at the competition.

  \begin{responsibilities}
  \item Coordination of lighting and music throughout, and between matches.
  \end{responsibilities}

\end{roledesc}

\begin{roledesc}{head-shepherd}
  The \roletitle{head-shepherd} ensures that teams turn up to their matches on time at the competition.
\end{roledesc}

\begin{roledesc}{commentators}
  The \roletitle{commentators} maintain a positive atmosphere during matches at the competition by providing commentary.
\end{roledesc}

\begin{roledesc}{head-match-scorer}
  The \roletitle{head-match-scorer} is responsible for scoring matches, and entering those scores into the competition software.
\end{roledesc}

\begin{roledesc}{judges}
  The \roletitle{judges} are responsible for making decisions throughout the competition where there is uncertainty about the result of a match.
 \end{roledesc}
